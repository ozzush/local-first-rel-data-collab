...
\makeatletter
\renewcommand{\ALG@name}{Specification}
\makeatother

The server's specification can be formalized as follows:

\begin{algorithm}
    \caption{Server}\label{alg:cap}
    \begin{minted}{kotlin}
    State:
        lastId: Id
    ...
    API:
        sendUpdate(update: Update, baseId: Id | Uid)
    
        synchronize(updates: List<Update>, baseId: Id): 
            returns (projectXml, lastId: Id, updatesApplied: Boolean)

        fetchUpdates(baseId: Id): 
            returns (updates: List<Updates>, lastId: Id)

        fetchProject():
            returns (projectXml: String, lastId: Id)
    \end{minted}
\end{algorithm}

When the server recieves a \verb|sendUpdate| call, it compares \verb|baseId| against \verb|lastId|. If the ids match, the server commits the update and notifies all connected clients about the update. If the ids don't match, the server notifies the author of the update about the failure.

When the server recieves a \verb|synchronize| call, it loads the state of the project at \verb|baseId| and checks for conflicts using Transaction Replay. If there are no conflicts, the server applies the updates to the latest state of the project. If there are conflicts, the updates are discarded. Finally, the server sends the state of the project to the client, setting the \verb|updatesApplied| field to indicate whether the client's updates were applied successfully.

TODO: \verb|fetchUpdates|, \verb|init()|
...
\begin{algorithm}
    ...
    State:
        ...
        lastId: Id | Uid
        project: ProjectDatabase
        unconfirmedUpdates: Queue<Update>
        server: ServerConnection

    Handlers:
        processLocalUpdate(update: Update):
            project.apply(update)
            unconfirmedUpdates.push(update)
            server.sendUpdate(update, lastId)
            lastId = update.id

        processServerUpdate(update: Update, baseId)
            if (baseId == lastId) {
                project.apply(update)
                lastId = update.id
                lastServerId = update.id
            } else if (update == unconfirmedUpdates.first()) {
                // confirmation for an unconfirmed update
                unconfirmedUpdates.pop()
                lastServerId = update.id
            } else {
                server.synchronize(unconfirmedUpdates.toList(), lastServerId)
            }

        processCommitFailure(failedUpdate: Update)
            server.synchronize(unconfirmedUpdates.toList(), lastServerId)
    \end{minted}
\end{algorithm}
